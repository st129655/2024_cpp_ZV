...
Matrix reverse(Matrix m1) {
	if (m1.cols == m1.rows) {
		Matrix res(m1.cols, m1.rows);
		for (int i = 0; i < m1.rows; ++i) {
			for (int j = 0; j < m1.cols; ++j) {
				Matrix temp = m1.minor(i, j);
				res.data[i][j] = temp.det();
			}
		}
		double c = 1 / m1.det();
		Matrix trans(m1.cols, m1.rows);
		for (int i = 0; i < m1.rows; ++i) {
			for (int j = 0; j < m1.cols; ++j) {
				trans.data[j][i] = res.data[i][j];
			}
		}
		for (int i = 0; i < m1.rows; ++i) {
			for (int j = 0; j < m1.cols; ++j) {
				trans.data[i][j] = trans.data[i][j] * c;
			}
		}
		for (int i = 0; i < m1.rows; ++i) {
			for (int j = 0; j < m1.cols; ++j) {
				if ((i + j) % 2 == 1) {
					trans.data[i][j] = trans.data[i][j] * -1;
				}
			}
		}
		return trans;
...
Matrix solve(Matrix A, Matrix B) {
	Matrix temp = reverse(A);
	Matrix res = mult(temp,B);
	return res;
}